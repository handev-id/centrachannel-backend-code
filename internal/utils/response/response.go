...
// sanitizeData guarantees array/map responses serialize as [] / {} instead of
// null when the underlying Go value is a nil slice or map (json.Marshal emits
// "null" for nil slices/maps). Explicit nil (data == nil) is preserved so
// delete/action endpoints can still return data: null on purpose.
func sanitizeData(data interface{}) interface{} {
	...
	case reflect.Slice:
		if v.IsNil() && v.Type().Elem().Kind() != reflect.Uint8 {
			// skip []byte / json.RawMessage — they serialize as a JSON string
